...
function Check() {
    ...
    setTimeout(() => {
        ...
        $configuration.sendMessage(mes1).then(resolve => {
            ...
            if (resolve.ret) {
                console.log("已经切换至被 <b>Google 送中</b> 的路线 ➟ "+NoList[1])
                content = `<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">` + "<br><b>⟦ "+$environment.params+ " ⟧ </b>已切换至被<b>Google</b> 送中的路线<br><br> 👇<br><br> ⟦ "+NoList[1]+ " ⟧" + "<br><br><font color=#FF5733>-----------------------------<br><b>检测详情请查看JS脚本记录</b><br>-----------------------------</font>"+`</p>`
                $done({"title":"Google 送中检测&切换", "htmlMessage": content })
            }
    }, reject => {
            $done();
